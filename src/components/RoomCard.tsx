...
export default function RoomCard({ room }: RoomCardProps) {
  ...
  const getDistrictUtility = (district: string) => {
    ...
    return 'Gần chợ, siêu thị tiện lợi ngang ngõ';
  };
      ...
      <div className="room-card bg-white rounded-2xl overflow-hidden shadow-md hover:shadow-xl border border-slate-100">
        ...
        <div className="p-4">
          ...
          <div className="mt-4 flex gap-2" onClick={(e) => e.preventDefault()}>
            <a href={`tel:${ZALO_PHONE}`} className="flex-1 py-2 bg-green-50 text-green-600 rounded-lg text-xs font-bold text-center hover:bg-green-100 transition-colors shadow-sm">
              📞 Gọi ngay
            </a>
            <a href={getZaloLink(ZALO_PHONE, `${DEFAULT_ZALO_MESSAGE} - ${room.title}`)} target="_blank" rel="noopener noreferrer" className="flex-1 py-2 bg-blue-50 text-blue-600 rounded-lg text-xs font-bold text-center hover:bg-blue-100 transition-colors shadow-sm">
              💬 Nhắn Zalo
            </a>
          </div>
        </div>
